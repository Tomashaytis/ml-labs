...
class LeaderClusterizer:
    def __init__(self, threshold: float, distance: str = 'euclidean', params: dict = None,
    ...
    def fit(self, sample: np.array):
        self._cluster_centers = []
        self._labels = []
        for x in sample:
            if len(self._cluster_centers) == 0:
                self._labels.append(len(self._cluster_centers))
                self._cluster_centers.append(x)
                ...
                if min_distance <= self._threshold:
                    self._labels.append(distances.index(min_distance))
                else:
                    self._labels.append(len(self._cluster_centers))
                    self._cluster_centers.append(x)
    ...
    def plot(self, sample: np.ndarray, true_labels: np.ndarray):
        ...
        axes[1].set_title('Простой алгоритм кластеризации')
        axes[1].legend()
