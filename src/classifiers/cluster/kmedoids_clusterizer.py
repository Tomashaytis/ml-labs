...
class KMedoidsClusterizer:
        ...
        while True:
            ...
            for _, cluster in clusters.items():
                ...
                for center in cluster:
                    distance_sums.append(0)
                    for x in cluster:
                        distance_sums[-1] += self.find_distance(center, x)
                new_centers.append(cluster[distance_sums.index(min(distance_sums))])
    ...
    def plot(self, sample: np.ndarray, true_labels: np.ndarray):
        ...
        axes[1].set_title('Алгоритм кластеризации К-Медоидов')
        axes[1].legend()
