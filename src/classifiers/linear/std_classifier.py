...
class STDClassifier:
    ...
    def fit(self, sample: np.array, true_labels: np.ndarray):
        ...
        sample_with_bias = np.column_stack([sample, ones])

        r = np.ones(true_labels.shape[0])
        r[true_labels == self._sample_param0.class_label] = -1
        r = r.reshape(-1, 1)
        sample_with_bias = (sample_with_bias * r).T

        gamma = np.ones(true_labels.shape[0]).reshape(-1, 1)

        weights_with_bias = np.linalg.inv(sample_with_bias @ sample_with_bias.T) @ sample_with_bias @ gamma
        self._weights = weights_with_bias[:-1]
        self._threshold = weights_with_bias[-1]
    ...
    def plot(self, sample: np.array, true_labels: np.ndarray = None):
        ...
        plt.title('Классификатор, минимизирующий СКО')
        plt.legend()
        plt.show()
