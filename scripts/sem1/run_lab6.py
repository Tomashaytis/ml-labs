...
from src.classifiers.cluster import Leader, Maximin, KMedoids, KMeans
...
CLUSTERIZER = 'kmeans'
DISTANCE = 'euclidean'
DATA_DIR = os.path.join('data', 'sem1', 'lab6')
...
if __name__ == '__main__':
    ...
    if CLUSTERIZER == 'leader':
        clusterizer_leader = Leader(THRESHOLD, DISTANCE, covariance=sample0.params.covariance)
        clusterizer_leader.fit(main_sample)
        clusterizer_leader.plot(main_sample, main_labels)

    if CLUSTERIZER == 'maximin':
        clusterizer_maximin = Maximin(N_CLUSTERS, COEF, DISTANCE, covariance=sample0.params.covariance)
    ...
    if CLUSTERIZER == 'kmedoids':
        clusterizer_kmedoids = KMedoids(N_CLUSTERS, DISTANCE, covariance=sample0.params.covariance)
        clusterizer_kmedoids.fit(main_sample, should_plot=True)
        clusterizer_kmedoids.plot(main_sample, main_labels)

        clusterizer_kmedoids = KMedoids(3, DISTANCE, covariance=sample0.params.covariance)
        clusterizer_kmedoids.fit(main_sample, should_plot=True)
        clusterizer_kmedoids.plot(main_sample, main_labels)
    ...
    if CLUSTERIZER == 'kmeans':
        clusterizer_kmeans = KMeans(N_CLUSTERS, DISTANCE, covariance=sample0.params.covariance)
        clusterizer_kmeans.fit(main_sample, should_plot=True)
        clusterizer_kmeans.plot(main_sample, main_labels)

        clusterizer_kmeans = KMeans(3, DISTANCE, covariance=sample0.params.covariance)
        clusterizer_kmeans.fit(main_sample, should_plot=True)
        clusterizer_kmeans.plot(main_sample, main_labels)
