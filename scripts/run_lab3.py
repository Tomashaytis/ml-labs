...
from src.classifiers.linear import FisherClassifier, STDClassifier, RobbinsMonroClassifier
from src.utils import complex_plot, classifiers_stats
...
if __name__ == '__main__':
    ...
    if CLASSIFIER == 'bayesian':
        classifier_bc = BayesianClassifier([sample0.params, sample1.params], is_equal_covariance=True)
        ...
        classifier_f = FisherClassifier(sample0.params, sample1.params, is_equal_covariance=True)
        classifier_f.plot(main_sample, main_labels)
        classifier_f.plot_with_lines(main_sample, COLORS, main_labels)
    elif CLASSIFIER == 'std':
        classifier_std = STDClassifier(sample0.params, sample1.params)
        ...
        classifier_rm = RobbinsMonroClassifier(sample0.params, sample1.params,
    ...
    if CLASSIFIER == 'bayesian':
        classifier_bc = BayesianClassifier([sample0.params, sample1.params], is_equal_covariance=False)
        classifier_bc.plot(main_sample, main_labels)
        classifier_bc.plot_with_lines(main_sample, COLORS, main_labels)
    elif CLASSIFIER == 'fisher':
        classifier_f = FisherClassifier(sample0.params, sample1.params, is_equal_covariance=False)
        classifier_f.plot(main_sample, main_labels)
        classifier_f.plot_with_lines(main_sample, COLORS, main_labels)
    elif CLASSIFIER == 'std':
        classifier_std = STDClassifier(sample0.params, sample1.params)
    ...
    if ROBBINS_MONRO_CHECK:
        ...
        for weights in start_weights:
            ...
            for b in np.linspace(0.5, 1, 10):
                classifier_rm = RobbinsMonroClassifier(sample0.params, sample1.params, is_equal_covariance=False,
                                                       is_min_std=IS_MIN_STD, b=b, start_weights=weights)
                classifier_rm.fit(main_sample, main_labels, epsilon=EPSILON)
